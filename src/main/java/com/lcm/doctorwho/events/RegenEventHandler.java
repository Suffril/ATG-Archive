...
@EventBusSubscriber public class RegenEventHandler { // NO_UCD (unused code)

	@SubscribeEvent public static void registerLoot(LootTableLoadEvent e) { // CHECK can this loot table actually be overriden in resource packs?
	...
	@SubscribeEvent public static void onAttacked(LivingAttackEvent e) {
		if (!(e.getEntity() instanceof EntityPlayer) || !SuperpowerHandler.hasSuperpower((EntityPlayer) e.getEntity(), TimelordSuperpower.INSTANCE))
			return;
	...
	@SubscribeEvent public static void onPlayerInteract(PlayerInteractEvent e) {
		if (e.getSide() == Side.CLIENT || !SuperpowerHandler.hasSuperpower(e.getEntityPlayer(), TimelordSuperpower.INSTANCE))
			return;
		if (SuperpowerHandler.getSpecificSuperpowerPlayerHandler(e.getEntityPlayer(), TimelordSuperpowerHandler.class).regenerating)
			e.setCanceled(true);
	}

	@SubscribeEvent public static void onKnockback(LivingKnockBackEvent e) {
		if (!(e.getEntity() instanceof EntityPlayer))
			return;
		EntityPlayer player = ((EntityPlayer) e.getEntity());
		if (!SuperpowerHandler.hasSuperpower(player, TimelordSuperpower.INSTANCE))
			return;
		if (SuperpowerHandler.getSpecificSuperpowerPlayerHandler(player, TimelordSuperpowerHandler.class).regenerating)
			e.setCanceled(true);
	}

	@SubscribeEvent public static void onLogin(PlayerEvent.PlayerLoggedInEvent e) {
	...
	@SubscribeEvent(priority = EventPriority.HIGH) public static void onHurt(LivingHurtEvent e) {
		...
		if ((handler.regenerating || player.posY < 0 || handler.regenerationsLeft == 0 || e.getSource() instanceof NoRegenDamageSource) && !ATGConfig.dontLoseUponDeath) {
			...
			if (handler.regenerationsLeft != -1)
				player.sendStatusMessage(new TextComponentString(StringHelper.translateToLocal("lcm-atg.messages.regenLeftExt", time, (handler.regenerationsLeft - 1))), true);
			...
			if (e.getSource() instanceof QuickRegenDamageSource)
				handler.regenTicks = 201;
		}
	}
