...
public class JsonGenUtil {
    ...
    public static void item(String itemName, String textureName) throws IOException {
        ...
        try (Writer writer = new OutputStreamWriter(new FileOutputStream(fileDir + "\\" + itemName + ".json"), "UTF-8");
    ...
    public static void block(String blockName, String textureName) throws IOException{
        ...
        Gson gson = new GsonBuilder().setPrettyPrinting().create();
        try (Writer writer = new OutputStreamWriter(new FileOutputStream(fileDir + "\\" + blockName + ".json"), "UTF-8");
            JsonWriter jw = gson.newJsonWriter(writer)) {
            jw.beginObject();
            jw.name("variants");
            jw.beginObject();
