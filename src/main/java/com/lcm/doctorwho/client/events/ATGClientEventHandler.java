		...
		if (worldClient != null)
		{
			...
			worldClient.loadedTileEntityList.forEach(tileEntity -> {
				if (tileEntity instanceof TileEntityTardis)
				{
					...
					camera.renderWorldToTexture(new Vec3d(tileEntity.getPos().getX(), tileEntity.getPos().getY(), tileEntity.getPos().getZ()),
					...
					if (Minecraft.getMinecraft().world.getTotalWorldTime() % 200 == 0)
					{ //TODO optimize
