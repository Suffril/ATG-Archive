...
public class TimelordSuperpower extends Superpower {
	...
	@SideOnly(Side.CLIENT) @Override public SuperpowerRenderer.ISuperpowerRenderer getPlayerRenderer() {
		if (renderhandler == null)
			renderhandler = new TimelordRenderHandler();
	...
	@Override @SideOnly(Side.CLIENT) public GuiCustomizer getCustomizerGui(EntityPlayer player) {
		return new GuiRegenCustomizer();
	}

	@Override @SideOnly(Side.CLIENT) public GuiScreen getAbilityGui(EntityPlayer player) {
	...
	@Override public int getCapsuleColor() {
	...
	public static class Injection extends InjectionSuperpower {
		...
		@Override public ItemStack inject(EntityPlayer player, ItemStack stack) {
			...
			if (handler != null)
				handler.regenerationsLeft = ATGConfig.regenCapacity;
