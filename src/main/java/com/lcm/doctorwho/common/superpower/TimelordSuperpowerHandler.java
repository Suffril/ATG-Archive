 ...
 */
@Mod.EventBusSubscriber public class TimelordSuperpowerHandler extends SuperpowerPlayerHandler {
	...
	@Override public void onUpdate(TickEvent.Phase phase) {
		...
		if (!player.world.isRemote) {
			...
			if (regenTicks > 0 && regenTicks < 200) { // regenerating
				...
				cap.syncToAll();
			} else if (regenTicks == 0 && regenerating)
				regenTicks = 1; // initiate regeneration
		} else {
			// Client Behavior
